...
public class PacketGetInfoFromServer implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        int id = buf.readInt();
        Class<? extends InfoPacketServer> clazz = AMPacketHandler.getServerInfoPacket(id);
        try {
    ...
    public void toBytes(ByteBuf buf) {
        buf.writeInt(AMPacketHandler.getServerInfoPacketId(packet.getClass()));
        packet.toBytes(buf);
    }
    ...
    private void sendReplyToClient(InfoPacketClient reply, EntityPlayerMP player) {
        AMPacketHandler.INSTANCE.sendTo(new PacketReturnInfoToClient(reply), player);
    }
}
