...
public class BundleBlock extends GenericBlockWithTE<BundleTE> {
    ...
    public void initItemModel() {
        Item itemBlock = GameRegistry.findItem(AquaMunda.MODID, "bundle");
        ModelResourceLocation itemModelResourceLocation = new ModelResourceLocation(AquaMunda.MODID + ":bundle", "inventory");
        ...
        for (CableSection section : bundleTE.getCableSections()) {
            ...
            if (networkId != -1) {
                ...
                if (accessor.getPlayer().isSneaking()) {
                    currenttip.add(EnumChatFormatting.YELLOW + section.getType().name() + ": " + section.getConnection(0) + " : " + bundleTE.getPos() + " : " + section.getConnection(1));
                }
            }
        }
